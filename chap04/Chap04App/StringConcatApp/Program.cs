...
namespace StringConcatApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("123" + p2);     // 123456 -> 문자열이 하나라도 있으면 문자열 결합으로 바뀜
            Console.WriteLine($"123{p2}");     // 결합연산자 없이 문자열 결합(C# 6.0 이후)
            ...
            int? d = null;
            Console.WriteLine($"{d ?? 0}"); // NULL이면 0을 출력(SQL : ISNULL)
        }
    }
}
